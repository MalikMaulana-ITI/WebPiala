<nav class="bg-white border-b border-gray-200 fixed w-full z-20 top-0 start-0 shadow-md">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
            <span class="self-center text-2xl font-semibold whitespace-nowrap text-green-700">PialaMurah.com</span>
        </a>

        <div class="flex-grow mx-4 hidden md:block">
            <div class="relative w-full">
                <input type="text" placeholder="Cari di PialaMurah.com..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-gray-800 text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            <div>
                ...
                <div class="relative mb-4">
                    <input type="text" placeholder="Search for categories" class="w-full bg-gray-100 text-gray-800 border border-gray-300 rounded-md py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                ...
                <div class="space-y-2 text-sm custom-scrollbar max-h-64 overflow-y-auto pr-2">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-gray-800">TV, Audio-Video</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Desktop PC</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Gaming</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Monitors</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Wearables</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded" checked>
                        <span class="ml-2 text-gray-800">Console</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded" checked>
                        <span class="ml-2 text-gray-800">Tablets</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Foto</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Fashion</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600 bg-gray-200 border-gray-300 rounded">
                        <span class="ml-2 text-gray-800">Books</span>
                    </label>
                    <a href="#" class="text-blue-600 hover:underline mt-2 block">See more &rarr;</a>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const TRANSITION_DURATION = 300; // Milliseconds, must match CSS transition duration

        openFilterBtn.addEventListener('click', function() {
            // Step 1: Make the overlay visible (opacity and visibility)
            filterOverlay.classList.add('show');
            // Step 2: After a slight delay, trigger the panel's slide-in transition
            setTimeout(() => {
                overlayPanel.classList.add('active');
            }, 10); // A very small delay to ensure 'show' class is applied first
            body.classList.add('no-scroll'); // Prevent body scroll
        });

        closeFilterBtn.addEventListener('click', function() {
            // Step 1: Start the panel's slide-out transition
            overlayPanel.classList.remove('active');
            // Step 2: After the panel animation finishes, fade out the overlay
            setTimeout(() => {
                filterOverlay.classList.remove('show');
                body.classList.remove('no-scroll'); // Allow body scroll
            }, TRANSITION_DURATION); // Wait for panel transition to finish
        });
        ...
        // Close overlay when clicking outside the panel (on the dark background)
        filterOverlay.addEventListener('click', function(event) {
            // Check if the click occurred directly on the overlay itself, not its children
            if (event.target === filterOverlay) {
                closeFilterBtn.click(); // Simulate click on close button
            }
        });
    });
